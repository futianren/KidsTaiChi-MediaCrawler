...
# Repository: https://github.com/futianren/KidsTaiChi-MediaCrawler/blob/main/store/xhs/__init__.py
...
def _xhs_note_persist_mode() -> str:
    return str(getattr(config, "XHS_NOTE_PERSIST_MODE", "full") or "full").strip().lower()


def _maybe_slim_xhs_local_item(local_db_item: Dict) -> Dict:
    """
    feishu_plus：在 json/jsonl/csv 落盘时减少字段体积（不减少详情 HTTP）。
    数据库类存储仍使用完整字段，避免列约束问题。
    """
    if _xhs_note_persist_mode() != "feishu_plus":
        return local_db_item
    if getattr(config, "SAVE_DATA_OPTION", "") not in ("json", "jsonl", "csv"):
        return local_db_item
    # 仅保留「飞书链路/去重/过滤依赖」字段
    keys = ("note_id", "title", "note_url", "type")
    return {k: local_db_item.get(k) for k in keys}
...
async def update_xhs_note(note_item: Dict):
    ...
    mode = _xhs_note_persist_mode()
    save_opt = getattr(config, "SAVE_DATA_OPTION", "")

    # 极简模式：仅保留飞书链路/去重/过滤依赖字段；并尽量避免无用字段计算
    if mode == "feishu_plus" and save_opt in ("json", "jsonl", "csv"):
        xsrc = (note_item.get("xsec_source") or "pc_search").strip() or "pc_search"
        xtok = note_item.get("xsec_token") or ""
        local_db_item = {
            "note_id": note_id,
            "title": note_item.get("title") or note_item.get("desc", "")[:255],
            "note_url": f"https://www.xiaohongshu.com/explore/{note_id}?xsec_token={xtok}&xsec_source={xsrc}",
        ...
        local_db_item = {
            ...
            "last_modify_ts": utils.get_current_timestamp(),  # Last modification timestamp
            "note_url": (
                f"https://www.xiaohongshu.com/explore/{note_id}"
                f"?xsec_token={note_item.get('xsec_token')}"
                f"&xsec_source={(note_item.get('xsec_source') or 'pc_search').strip() or 'pc_search'}"
            ),
            "source_keyword": source_keyword_var.get(),  # Search keyword
            "xsec_token": note_item.get("xsec_token"),  # xsec_token
        }
    persist_item = _maybe_slim_xhs_local_item(local_db_item)
    utils.logger.info(f"[store.xhs.update_xhs_note] xhs note: {persist_item}")
    await XhsStoreFactory.create_store().store_content(persist_item)
    try:
        from tools.feishu import xhs_feishu_sink

        await xhs_feishu_sink.enqueue(
            local_db_item.get("note_id"),
            local_db_item.get("title"),
            local_db_item.get("note_url"),
        )
    except Exception as exc:
        utils.logger.warning("[store.xhs.update_xhs_note] 飞书 enqueue 失败（已忽略）: %s", exc)
